...
# import librairie des mots fr de spacy
...
###############################################################################
# DESCRIPTION:
# Fonction permettant de nettoyer partiellement le texte en enlevant tout les
# caracteres spéciaux (, ; . ? : /)
...
def clean(contenu):
    ...
    return contenu
# DESCRIPTION:
# Fonction permettant de ressortir les chiffres importants pour notre étude et
# d'avoir ,pour chaque chiffre, le mots associé à ce dernier. Il modifie les
# textes pour éviter des erreurs liées aux nombres.
...
# DESCRIPTION:
# Fonction permettant d'afficher l'heure et connaitre l'avancement du programme
# ENTREE: text (string) //text pour savoir l'etape du programme executé
# SORTIE: x
...
# DESCRIPTION:
# les dates peuvent etre de type différents, vides ou dans le mauvais format
...
#         1) nettoyer texte
#         2) recuperer tout les mots distint et d'avoir les infos sur eux
...
# ENTREE: type_ (string) //'title' ou 'content'
#        all_ list) // liste sauvegardant tout les texte nettoyés
#        export (array) //tableau contenant les infos relative aux tables
#                           "contenus_mots_cles" et "titres"
#        vec (TfidfVectorizer) //fonction tfidf paramètrée
#        tfidf (array) //tableau des tfidf titre et contenu differents
#        ID (int) //Identifiant pour la table "titres" et "contenus_mot_clé"
# SORTIE: x
...
# DESCRIPTION:
# Fonction permettant d'attribuer les clé étrangère des mots aux nombres
# ENTREE: x
# SORTIE: x
...
# DESCRIPTION:
# Fonction permettant d'exporter les tableaux relatif aux table en csv
...
for j in range(0, len(daily_name)):
    ...
    for i in range(1, daily_number[j] + 1):
        ...
        ID_numbers = Extract('content', file["content"], all_doc, list_word,
                             details_word, ID_numbers, i, export_numbers)
